...
\chapter{Future work}
At the moment of writing this document, the identified future work in a very short term is finishing the Linux image creation, containing a WebKitGTK+ web browser for ARMv7 processor target, running it in the QEMU emulator.

Once the planned work is completed the following work would be testing this compiled image in a real embedded device target, verifying its behaviour in a real environment.

[TODO]

Finish the image creation.

Test the image in a real target.

Create a patch to be sent to meta-browser layer project.

Integration with OSTree\cite{ostree}.
